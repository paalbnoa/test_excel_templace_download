...
import { FIELD_RULES, validateNorwegianPersonalNumber } from "./validation-rules.js";

export const SEMESTER_OPTIONS = ["2025H", "2026V", "2026H"];

const INSTITUTION_SHORT_NAME_LABEL = "Institution short name:";
const METADATA_LABEL_COLUMN_WIDTH = 23;

export const COLUMNS = [
  { header: "PersonID", key: "personId", width: METADATA_LABEL_COLUMN_WIDTH },
...
const EXTRA_READY_ROWS = 1000;
const TEST_DATA_ROWS = 100;
const HEADER_ROW = 6;
...
function addMetadataFields(worksheet, schoolName, selectedSemesters) {
  ...
  worksheet.getCell("A3").value = INSTITUTION_SHORT_NAME_LABEL;
...
function addStudentsTable(worksheet, numRows) {
  worksheet.addTable({
    ...
    rows: Array.from({ length: numRows }, () => COLUMNS.map(() => ""))
  });
}
...
function buildDataValidation(fieldRule, cellReference) {
  return {
    ...
    error: fieldRule.excel.errorMessage ?? fieldRule.errorMessage,
...
function applyColumnValidations(worksheet, lastReadyRow) {
  COLUMNS.forEach(({ header }, index) => {
    ...
    const range = `${firstCell}:${col}${lastReadyRow}`;
    const prefix = `${header}: `;
    const prompt = rule.instruction.startsWith(prefix)
      ? rule.instruction.slice(prefix.length)
      : rule.instruction;

    worksheet.dataValidations.add(range, {
      ...buildDataValidation(rule, firstCell),
...
function randomNorwegianPersonalNumberBirthDate() {
  ...
  const earliestBirthDate = Date.UTC(
    today.getUTCFullYear() - 100,
    today.getUTCMonth(),
    today.getUTCDate() + 1
  );
  const totalDays = (latestBirthDate - earliestBirthDate) / MILLISECONDS_PER_DAY + 1;
  const dayOffset = Math.floor(Math.random() * totalDays);

  return new Date(earliestBirthDate + dayOffset * MILLISECONDS_PER_DAY);
}

function buildNorwegianIndividualNumberCandidates(year) {
  const ranges = [];
...
export function generateNorwegianPersonalNumber() {
  while (true) {
    // Pick a random birth date inside the supported Norwegian number date range.
    const birthDate = randomNorwegianPersonalNumberBirthDate();
    const day = padTwoDigits(birthDate.getUTCDate());
    const month = padTwoDigits(birthDate.getUTCMonth() + 1);
    const year = birthDate.getUTCFullYear();
    const shortYear = padTwoDigits(year % 100);

    // Format the date portion as DDMMYY.
    const datePart = `${day}${month}${shortYear}`;

    // Build the individual-number candidates that are valid for this century.
    const individualNumbers = shuffleItems(buildNorwegianIndividualNumberCandidates(year));

    // Try the candidates in random order until both check digits are valid.
    for (const individualNumber of individualNumbers) {
      // Pad III to three digits; the last digit encodes gender, odd for male and even for female.
      const individualPart = String(individualNumber).padStart(3, "0");
      const firstNineDigits = `${datePart}${individualPart}`;

      // Calculate K1 from the first 9 digits and skip invalid K1 combinations.
      const k1 = calculateNorwegianCheckDigit(firstNineDigits, K1_WEIGHTS);
      if (k1 === null) {
        continue;
      }

      // Calculate K2 from the first 10 digits, including K1, and skip invalid K2 combinations.
      const firstTenDigits = `${firstNineDigits}${k1}`;
      const k2 = calculateNorwegianCheckDigit(firstTenDigits, K2_WEIGHTS);
      if (k2 === null) {
        continue;
      }

      const personalNumber = `${firstTenDigits}${k2}`;

      if (validateNorwegianPersonalNumber(personalNumber)) {
        // Return the complete synthetic personal number as an 11-character string.
...
function buildTestRow(index) {
  ...
  const personId = generateNorwegianPersonalNumber();
...
function configureDataSheet(worksheet, schoolName, selectedSemesters, options = {}) {
  ...
  const numRows = options.numRows ?? EXTRA_READY_ROWS;
  const lastReadyRow = FIRST_DATA_ROW + numRows - 1;

  addStudentsTable(worksheet, numRows);
  configureHeaderRow(worksheet);

  const lastTestDataRow = FIRST_DATA_ROW + TEST_DATA_ROWS - 1;
  for (let rowNumber = FIRST_DATA_ROW; rowNumber <= lastReadyRow; rowNumber += 1) {
    const row = configureDataRowCells(worksheet, rowNumber);

    if (options.includeTestData && rowNumber <= lastTestDataRow) {
      populateTestRow(row, rowNumber - FIRST_DATA_ROW);
    }
  }

  applyColumnValidations(worksheet, lastReadyRow);
  ...
  worksheet.getColumn(1).width = METADATA_LABEL_COLUMN_WIDTH;
}
...
function configureInstructionSheet(worksheet, selectedSemesters, options = {}) {
  ...
  const instructionRows = [
    ...
    "",
    "Other notes:",
    "1. The table contains 1000 blank rows ready for data entry."
  ];
