...
function configureDataRowCells(worksheet, rowNumber) {
  ...
  worksheet.getCell(`A${rowNumber}`).dataValidation = buildDataValidation(
    FIELD_RULES.PersonID,
    `A${rowNumber}`
  );
  worksheet.getCell(`B${rowNumber}`).dataValidation = buildDataValidation(
    FIELD_RULES.Fornavn,
    `B${rowNumber}`
  );
  worksheet.getCell(`C${rowNumber}`).dataValidation = buildDataValidation(
    FIELD_RULES.Etternavn,
    `C${rowNumber}`
  );
  worksheet.getCell(`D${rowNumber}`).dataValidation = buildDataValidation(
    FIELD_RULES["Fritatt.sem.avg"],
    `D${rowNumber}`
  );
  worksheet.getCell(`E${rowNumber}`).dataValidation = buildDataValidation(
    FIELD_RULES.Epost,
    `E${rowNumber}`
  );
  worksheet.getCell(`F${rowNumber}`).dataValidation = buildDataValidation(
    FIELD_RULES.Prefiks,
    `F${rowNumber}`
  );
  worksheet.getCell(`G${rowNumber}`).dataValidation = buildDataValidation(
    FIELD_RULES.Mobilnummer,
    `G${rowNumber}`
  );
