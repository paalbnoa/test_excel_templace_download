...
function buildTestRow(index, selectedSemesters) {
  const rowNumber = index + 1;
  const semester = selectedSemesters[index % selectedSemesters.length];
  const personId = 100000 + rowNumber;
  const mobileBase = 40000000 + rowNumber;

  return {
    personId,
    fornavn: `Test${rowNumber}`,
    etternavn: `Student${rowNumber}`,
    sistBetSemAvg: semester,
    fritattSemAvg: "",
...
function populateTestRow(row, selectedSemesters, index) {
  const sample = buildTestRow(index, selectedSemesters);

  row.getCell(1).value = sample.personId;
  row.getCell(2).value = sample.fornavn;
  row.getCell(3).value = sample.etternavn;
  row.getCell(4).value = sample.sistBetSemAvg;
  row.getCell(5).value = sample.fritattSemAvg;
  row.getCell(6).value = sample.epost;
  row.getCell(7).value = sample.prefiks;
  row.getCell(8).value = sample.mobilnummer;
}
...
function configureDataSheet(worksheet, schoolName, selectedSemesters, options = {}) {
  ...
  for (let rowNumber = FIRST_DATA_ROW; rowNumber <= LAST_READY_ROW; rowNumber += 1) {
    ...
    if (options.includeTestData) {
      populateTestRow(row, selectedSemesters, rowNumber - FIRST_DATA_ROW);
    }
  }
