...
const NORWEGIAN_PERSONAL_NUMBER_START_DATE = Date.UTC(1854, 0, 1);
const NORWEGIAN_PERSONAL_NUMBER_END_DATE = Date.UTC(2039, 11, 31);
...
function randomNorwegianPersonalNumberBirthDate() {
  const totalDays =
    (NORWEGIAN_PERSONAL_NUMBER_END_DATE - NORWEGIAN_PERSONAL_NUMBER_START_DATE) /
      MILLISECONDS_PER_DAY +
    1;
  const dayOffset = Math.floor(Math.random() * totalDays);

  return new Date(NORWEGIAN_PERSONAL_NUMBER_START_DATE + dayOffset * MILLISECONDS_PER_DAY);
}
