...
const PERSON_ID_FULL_ERROR_MESSAGE =
...
const PERSON_ID_EXCEL_ERROR_MESSAGE =
  "PersonID is required and must contain exactly 11 digits.";

function buildLettersOnlyFormula(cellReference) {
  const c = cellReference;
  return `AND(LEN(TRIM(${c}))>0,SUMPRODUCT(--(ISNUMBER(1*MID(TRIM(${c}),ROW($1:$50),1))))=0)`;
}

function buildPersonIdFormula(cellReference) {
  return `AND(LEN(${cellReference}&"")=11,SUMPRODUCT(--ISNUMBER(FIND(MID(${cellReference}&"",ROW($1:$11),1),"0123456789")))=11)`;
}

function buildPrefixFormula(cellReference) {
  const c = cellReference;
  return `AND(LEFT(${c},1)="+",LEN(${c})>1,LEFT(${c},3)<>"+00",COUNTIF(${c},"* *")=0,SUMPRODUCT(--(ISNUMBER(1*MID(RIGHT(${c},LEN(${c})-1),ROW($1:$20),1))))=LEN(${c})-1)`;
}
...
function isYoungerThan100Years(year, month, day) {
  const today = new Date();
  const cutoffYear = today.getUTCFullYear() - 100;
  const cutoffMonth = today.getUTCMonth() + 1;
  const cutoffDay = today.getUTCDate();
...
export function validateNorwegianPersonalNumber(value) {
  ...
  const hasValidBirthDateAndIndividualNumber = candidateYears.some((year) => {
    return (
      ...
      isYoungerThan100Years(year, month, day)
    );
  });
...
export const FIELD_RULES = {
  PersonID: {
    instruction:
      "PersonID: This field is required and must be a valid 11-digit Norwegian personal number. Person must be under 100 years old. The first 6 digits must be a real birth date, followed by a valid individual number for the birth year and two valid MOD11 check digits.",
    errorTitle: "Invalid PersonID",
    errorMessage: PERSON_ID_FULL_ERROR_MESSAGE,
    excel: {
      type: "custom",
      allowBlank: false,
      errorMessage: PERSON_ID_EXCEL_ERROR_MESSAGE,
      formula: buildPersonIdFormula
    },
    validate: (value) => validateNorwegianPersonalNumber(value)
  },
  Fornavn: {
    instruction: "Fornavn: This field is required and must contain letters only. Spaces are allowed.",
    errorTitle: "Invalid Fornavn",
    errorMessage: "Fornavn is required and must contain letters only. Spaces are allowed.",
    excel: {
      type: "custom",
      allowBlank: false,
  ...
  Etternavn: {
    instruction:
      "Etternavn: This field is required and must contain letters only. Spaces are allowed.",
    errorTitle: "Invalid Etternavn",
    errorMessage: "Etternavn is required and must contain letters only. Spaces are allowed.",
    excel: {
      type: "custom",
      allowBlank: false,
      formula: (cellReference) => buildLettersOnlyFormula(cellReference)
